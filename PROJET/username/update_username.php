...
$username_err = $password_err = $email_err = $admin_err = "";
...
if(isset($_POST["id"]) && !empty($_POST["id"])){
    ...
    if(empty($input_email)){
        $email_err = "Please enter the a valid email.";     
    } else{
        $email = $input_email;
    }
    ...
    if(empty($username_err) && empty($password_err) && empty($email_err) && empty($admin_err)){
        ...
        $sql = "UPDATE users SET username=?, password=?, email=?, admin=? WHERE id=?";
 
        if($stmt = $mysqli->prepare($sql)){
            // Bind variables to the prepared statement as parameters
            $stmt->bind_param("sssii", $param_username, $param_password, $param_email, $param_admin, $param_id);
